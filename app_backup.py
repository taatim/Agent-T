            ...
            try:
                ...
                if data.get("type") == "input":
                    ...
                    if call_agents and text_to_speak:
                        call_connection_id = list(call_agents.keys())[0]
...
async def initiate_call():
    ...
    result = acs_client.create_call(
        ...
        cognitive_services_endpoint=os.getenv("AZURE_OPENAI_SERVICE_ENDPOINT") # Assuming Speech resource is linked/same region often, but ideally should be speech endpoint if different. 
...
async def callback_handler(request: Request):
    ...
    for event in events:
        ...
        if event_type == 'Microsoft.Communication.IncomingCall':
            ...
            cog_endpoint = os.getenv("AZURE_OPENAI_SERVICE_ENDPOINT")
            
            # Additional Log to ensure we aren't passing None
            logger.info(f"Using Cognitive Services Endpoint: {cog_endpoint}")
        ...
        if event['type'] == 'Microsoft.Communication.CallConnected':
            ...
            # STT finished
            if event.get('recognitionType') == 'speech':
               text = event['speechResult']['speech']
...
async def play_to_call(call_connection_id, text):
    ...
    text_source = TextSource(text=text, voice_name="en-US-JennyNeural")
